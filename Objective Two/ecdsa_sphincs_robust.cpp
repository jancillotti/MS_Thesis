...
static int sign_and_verify_ecdsa_sphincs_robust() {
    ...
    auto priv_key = Botan::SphincsPlus_PrivateKey(rng,
        Botan::Sphincs_Parameter_Set::SLHDSA128Small, Botan::Sphincs_Hash_Type::Sha256);
...
int run_ecdsa_sphincsr_benchmark(std::ostream &out) {
    ...
    for (size_t msg_size: message_sizes) {
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            std::cout << "ECDSA + SPHINCS+ Robust i=" << i << "\t";
            sign_and_verify_ecdsa_sphincs_robust();
        }
        ...
        out << "\n";
        out << "=== ECDSA + SPHINCS+ Robust Results ===\n";
